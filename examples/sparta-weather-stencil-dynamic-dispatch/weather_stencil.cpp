...
// Harcoding where the DUs start
std::vector du_starting_columns{2, 10, 18, 26, 34, 42};
...
hsa_status_t air_load_airbin(hsa_agent_t agent, hsa_queue_t *q,
                             const char *filename, uint8_t column, airbin_table_entry* & existing_airbin_table) {
  ...
  // If we pass an existing airbin table, we want to use that to configure
  // the columns
  if(existing_airbin_table == NULL) {
    ...
    */
    ...
    // Caching this one so we can use it later
    existing_airbin_table = airbin_table;
  }
...
/*
  Launch an AIRBIN spatially across `num_spatial_launches` DUs
*/
hsa_status_t kernel_config(hsa_agent_t agent, hsa_queue_t *q, const char *filename, uint8_t num_spatial_launches) {

  // Making sure that the number of spatial launches is supported
  if(num_spatial_launches > du_starting_columns.size()) {
    std::cerr << "Attempted to launch across " << num_spatial_launches << " segments but device only supports " << du_starting_columns.size() << " DUs" << std::endl;
    return HSA_STATUS_ERROR_OUT_OF_RESOURCES;
  }

  // Reuse the airbin table across kernel launches
  airbin_table_entry* airbin_table = NULL;

  // Configuring the device
  for(int i = 0; i < num_spatial_launches; i++) { 
    hsa_status_t airbin_ret = air_load_airbin(agent, q, "sparta-1DU.elf", du_starting_columns[i], airbin_table);
    if (airbin_ret != HSA_STATUS_SUCCESS) {
      std::cerr << "Loading airbin in column " << du_starting_columns[i] << " failed: " << airbin_ret << std::endl;
      hsa_queue_destroy(q);
      hsa_shut_down();
      return airbin_ret;
    }
...
int main(int argc, char *argv[]) {
  ...
  // Configure the kernel across `num_spatial_launches` DUs
  auto kernel_config_ret = kernel_config(agent, queue, "sparta-1DU.elf", num_spatial_launches);
  if (kernel_config_ret != HSA_STATUS_SUCCESS) {
    std::cerr << "Loading airbin failed: " << kernel_config_ret << std::endl;
  ...
  auto allocate_and_init_output_buffer = [&] {
    ...
    for(int i = 0; i < num_spatial_launches; i++) {
      hsa_amd_memory_pool_allocate(global_mem_pool,
                                 ...
                                 (void **)&ddr_ptr_out[i]);
      // Fill with 0
      std::fill(ddr_ptr_out[i], ddr_ptr_out[i] + dma_count_in, 0);
    }
    return ddr_ptr_out;
  };
  ...
  hsa_amd_signal_create_on_agent(8 * num_spatial_launches, 0, nullptr, &agent, 0, &dma_signal);

  uint64_t wr_idx, packet_id;

  // Send the data for each spatial launch
  for(int i = 0; i < num_spatial_launches; i++) {
    ...
    hsa_agent_dispatch_packet_t pkt;
    air_packet_nd_memcpy(&pkt, 0, du_starting_columns[i], 1, 0, 4, 2,
    ...
    air_packet_nd_memcpy(&pkt2, 0, du_starting_columns[i], 0, 0, 4, 2,
                         reinterpret_cast<uint64_t>(ddr_ptr_out_0[i]),
    ...
    air_packet_nd_memcpy(&pkt3, 0, du_starting_columns[i], 1, 1, 4, 2,
    ...
    air_packet_nd_memcpy(&pkt4, 0, du_starting_columns[i], 0, 1, 4, 2,
                         reinterpret_cast<uint64_t>(ddr_ptr_out_1[i]),
                         dma_count_out * sizeof(float), 1, 0, 1, 0, 1, 0);
    ...
    air_packet_nd_memcpy(&pkt5, 0, du_starting_columns[i]+1, 1, 0, 4, 2,
    ...
    air_packet_nd_memcpy(&pkt6, 0, du_starting_columns[i]+1, 0, 0, 4, 2,
                         reinterpret_cast<uint64_t>(ddr_ptr_out_2[i]),
                         dma_count_out * sizeof(float), 1, 0, 1, 0, 1, 0);
    ...
    air_packet_nd_memcpy(&pkt7, 0, du_starting_columns[i]+1, 1, 1, 4, 2,
    ...
    air_packet_nd_memcpy(&pkt8, 0, du_starting_columns[i]+1, 0, 1, 4, 2,
                         reinterpret_cast<uint64_t>(ddr_ptr_out_3[i]),
                         dma_count_out * sizeof(float), 1, 0, 1, 0, 1, 0);
  ...
  int errors = 0;
  int errors_two = 0;
  for(int kernel_run = 0; kernel_run < num_spatial_launches; kernel_run++) {
    for (int i = 0; i < 512; i++) {
      if (ddr_ptr_out_0[kernel_run][i] != 514 + i) {
        std::cerr << "[ERROR] " << ddr_ptr_out_0[kernel_run][i] << " != " << 514 + i << std::endl;
        errors++;
      }

      if (ddr_ptr_out_0[kernel_run][i] != ddr_ptr_out_1[kernel_run][i]) {
        std::cerr << "[ERROR] ddr_ptr_out_0[" << i << "] (" << ddr_ptr_out_0[kernel_run][i] << ")  != ddr_ptr_out_1[" << i << "] (" << ddr_ptr_out_1[kernel_run][i] << ")" << std::endl;
        errors++;
      }

      if (ddr_ptr_out_0[kernel_run][i] != ddr_ptr_out_2[kernel_run][i]) {
        std::cerr << "[ERROR] ddr_ptr_out_0[" << i << "] (" << ddr_ptr_out_0[kernel_run][i] << ")  != ddr_ptr_out_2[" << i << "] (" << ddr_ptr_out_2[kernel_run][i] << ")" << std::endl;
        errors++;
      }

      if (ddr_ptr_out_0[kernel_run][i] != ddr_ptr_out_3[kernel_run][i]) {
        std::cerr << "[ERROR] ddr_ptr_out_0[" << i << "] (" << ddr_ptr_out_0[kernel_run][i] << ")  != ddr_ptr_out_3[" << i << "] (" << ddr_ptr_out_3[kernel_run][i] << ")" << std::endl;
        errors++;
      }
    }

    printf("Checking output of spatial launch %d\n", kernel_run);
    if (!errors) {
      print_weather(true);
    } else {
      print_weather(false);
  ...
  for(int i = 0; i < num_spatial_launches; i++) {
    hsa_amd_memory_pool_free((void *)ddr_ptr_out_0[i]);
    hsa_amd_memory_pool_free((void *)ddr_ptr_out_1[i]);
    hsa_amd_memory_pool_free((void *)ddr_ptr_out_2[i]);
    hsa_amd_memory_pool_free((void *)ddr_ptr_out_3[i]);
  }
