...
We provide a configuration file `sparta-1DU.elf` which contains the configuration of a 48-AIE design in a single DU of the VCK5000. We also provide the host code in `weather_stencil.cpp` which utilizes the converged ROCm runtime to communicate with the device, configure the AIEs with this design, perform memory allocation, etc. This application requires the [experimental converged ROCm runtime](https://github.com/RadeonOpenCompute/ROCR-Runtime/tree/experimental/rocm-5.6.x-air). This application also requires libelf and elfutils. Please run `apt install libelf-dev elfutils` to install these tools. When this is done, you must set two environment variables: `ROCM_ROOT` and `ELFUTILS_ROOT`. `ROCM_ROOT` should be set to the local install of the experimental ROCm runtime. `ELFUTILS_ROOT` is by default installed in `/usr/lib/x86_64-linux-gnu`. Users can run `dpkg -L libelf-dev` to find the location. Now, once ROCm and elfutils are installed and the environment variables are set, run the following commands to compile and run the weather stencil on the AIEs on the VCK5000:

```
make
./bp_weather_stencil.exe
```

In this version of the application, the airbin loading is handled by the ARM cores and the DMA packet processing is handled by one of the BP cores.
