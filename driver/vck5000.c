...
static void vck5000_init_queues(struct amdair_device *air_dev)
{
	...
	dev_info(&air_dev->pdev->dev,
		 "%s: Initializing queues, queue map %lx "
		 "queue size %llx, queue buf size %llx",
		 __func__, *air_dev->queue_mgr.hw_queue_map, queue_size,
		 queue_buf_size);
}
...
static void vck5000_init_doorbells(struct amdair_device *air_dev)
{
	...
	dev_info(&air_dev->pdev->dev,
		 ...
		 "doorbell map %lx, num db pages %d",
		 __func__, air_dev->doorbell.size,
		 *air_dev->doorbell.page_id_map,
		 air_dev->doorbell.num_db_pages);
}
...
{
	vck5000_bram_reg_write64(
		air_dev, DRAM_HEAP_VADDR_REG + queue_reg_off * sizeof(uint64_t),
		vaddr);
}
						  ...
						  uint64_t *arg, int num_args)
{
	...
	iowrite64(completion_signal_val,
		  air_dev->queue_mgr.admin_queue.ring_buf +
			  pkt_id * AQL_PKT_SIZE +
	...
	completion_signal_val = ioread64(
		air_dev->queue_mgr.admin_queue.ring_buf +
	...
	if (cmd_type == AQL_AIR_PKT_TYPE_READ_AIE_REG32) {
		arg[num_args] =
			ioread32(air_dev->queue_mgr.admin_queue.ring_buf +
				 pkt_id * AQL_PKT_SIZE + AQL_PKT_ARG_OFFSET +
				 num_args * sizeof(uint64_t));
	}
}
